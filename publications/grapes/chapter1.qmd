...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Always show *something* even if auth fails
  gateDiv.innerHTML = `
    <div style="background:#e2e3e5;border:1px solid #d3d6d8;border-radius:8px;padding:16px;margin:1rem 0;">
      Checking sign-in status...
    </div>
  `;

  try {
    const authStatus = await window.checkAuth();
    ...
    gateDiv.innerHTML = `
      <div style="background:#f8d7da;border:1px solid #f5c2c7;border-radius:8px;padding:16px;margin:1rem 0;">
        <strong>Auth error:</strong> ${String(err?.message || err)}<br/>
        Open the browser console for details.
      </div>
    `;

    // Optional during debugging: show content even if auth fails
    // contentDiv.style.display = 'block';
  }
});
</script>
